...
const TableOrder = () => {
  const location = useLocation();
    ...
    const addItem = async (id, price, quantity) => {
      ...
      const invoicedatas = await getDocs(query(collection(db, "Invoice") ,and(where("tableid", "==", Number(userData.tableNo)), where("status", "==", 0) )))
          const res = [];
     ...
     useEffect(() => {
      ...
      const handleCategoryClick = (category, id) => {
        ...
        if (category === "All") {
          ...
          setFilteredItems(items.filter((item) => item.categoryId === id));
        }
      };
            ...
            <Row className='Tableorder-bg-container-inside mb-1'>
                <Col sm={9} className='text-start'>
                    <Col sm={12} className='text-start'>
                        <TableHeader tableNp={userData.tableNo} userId={userData.userId}/>
                    </Col>
                    <Col sm={12} className='text-start'>
                        <div className="p-1">
                            {/* Category Cards */}
